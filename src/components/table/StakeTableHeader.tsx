...
export const StakeTableHeader = ({ 
  ...
  return (
    <TableHeader>
      <TableRow>
        <TableHead className="w-1/4" onClick={() => onSort('stake_account')}>
          Stake Account {renderSortIndicator('stake_account')}
        </TableHead>
        <TableHead className="w-[12%]" onClick={() => onSort('sol_balance')}>
          SOL Balance {renderSortIndicator('sol_balance')}
        </TableHead>
        <TableHead className="w-[12%]" onClick={() => onSort('status')}>
          Status {renderSortIndicator('status')}
        </TableHead>
        <TableHead className="w-[12%]" onClick={() => onSort('active_stake_amount')}>
          Active Stake {renderSortIndicator('active_stake_amount')}
        </TableHead>
        <TableHead className="w-[12%]" onClick={() => onSort('total_reward')}>
          Rewards {renderSortIndicator('total_reward')}
        </TableHead>
        <TableHead className="w-1/4" onClick={() => onSort('voter')}>
